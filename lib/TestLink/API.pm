...
use Scalar::Util qw{reftype looks_like_number}
  ; #boo, some functions return hashes and arrays depending on # of results (1 or many)
...
sub new {
    ...
    my $self = {
        ...
        flattree => [],
        invalidate_cache =>
          1 #since we don't have a cache right now... #TODO this should be granular down to project rather than global
    };
...
sub AUTOLOAD {
    my %public_elements =
      map { $_, 1 } qw{apiurl apikey};    #Public element access
    our $AUTOLOAD;
    ...
    if ( $AUTOLOAD =~ /::(\w+)$/ and exists $public_elements{$1} ) {
        my $field = $1;
        {
            no strict 'refs';
            *{$AUTOLOAD} = sub {
                confess("Object parameters must be called by an instance")
                  unless ref( $_[0] );
...
sub createTestSuite {
    ...
    confess("Object parameters must be called by an instance")
      unless ref($self);
    confess("Parent Project ID (arg 0) must be an integer")
      unless looks_like_number($project_id);
...
sub createTestCase {
    my (
        $self,        $test_name, $test_suite_id, $test_project_id,
        $author_name, $summary,   $steps,         $preconditions,
        $execution,   $order
    ) = @_;
    confess("Object parameters must be called by an instance")
      unless ref($self);
...
sub reportTCResult {
    my (
        $self,   $case_id,  $plan_id, $build_id,
        $status, $platform, $notes,   $bugid
    ) = @_;
    confess("Object parameters must be called by an instance")
      unless ref($self);
...
sub addTestCaseToTestPlan {
    my (
        $self,    $plan_id,  $case_id, $project_id,
        $version, $platform, $order,   $urgency
    ) = @_;
    confess("Object parameters must be called by an instance")
      unless ref($self);
...
sub uploadExecutionAttachment {
    my ( $self, $execution_id, $filename, $filetype, $content, $title,
        $description )
      = @_;
    confess("Object parameters must be called by an instance")
      unless ref($self);
...
sub uploadTestCaseAttachment {
    my ( $self, $case_id, $filename, $filetype, $content, $title, $description )
      = @_;
    confess("Object parameters must be called by an instance")
      unless ref($self);
...
sub getProjects {
    ...
    if ( !scalar( @{ $self->{'testtree'} } ) ) {
        $self->{'testtree'} = $result->result
          unless $result->result->[0]->{'code'};
    }
...
sub getTLDTestSuitesForProject {
    ...
    my $rpc = XMLRPC::Lite->proxy( $self->apiurl );
    my $result =
      $rpc->call( 'tl.getFirstLevelTestSuitesForTestProject', $input );
    ...
    if (
        $result->result
        && !(
               reftype( $result->result ) eq 'HASH'
            || reftype( $result->result ) eq 'ARRAY'
        )
      )
    {
        return [];
    ...
    @$res = values( %{ $result->result } )
      if reftype( $result->result ) eq 'HASH'
      && !defined( $result->result->{'id'} );
    $res = [ $result->result ]
      if reftype( $result->result ) eq 'HASH'
      && defined( $result->result->{'id'} );
    return []
      if ( !scalar( keys( %{ $res->[0] } ) ) )
      ;    #Catch bizarre edge case of blank hash being only thing there

    if ($get_tests) {
        for ( my $i = 0 ; $i < scalar( @{ $result->result } ) ; $i++ ) {
            $result->result->[$i]->{'tests'} =
              $self->getTestCasesForTestSuite( $result->result->[$i]->{'id'},
                0, 1 );
        }
    }
...
sub getTestSuitesForTestSuite {
    ...
    if ($get_tests) {
        foreach my $row (@$res) {
            $row->{'tests'} =
              $self->getTestCasesForTestSuite( $row->{'id'}, 0, 1 );
        }
    }
...
sub getTestSuitesByName {
    ...
    $self->_cacheProjectTree( $project_id, 1, 1, 0 )
...
sub getLatestBuildForTestPlan {
    ...
    my $input = {
        devKey  => $self->apikey,
        tplanid => $plan_id
        ,    #documented arg, but that's LIES, apparently it wants the next one
        testplanid => $plan_id
    };
...
sub _cacheProjectTree {
    ...
    for my $projhash (@flattener) {
        if ($use_project_id) {
            next
              if $project
              && $project ne $projhash->{'id'}
              && ( defined( $projhash->{'type'} )
                && $projhash->{'type'} eq 'project' );
        }
        else {
            next
              if $project
              && $project ne $projhash->{'name'}
              && ( defined( $projhash->{'type'} )
                && $projhash->{'type'} eq 'project' );
        }
        ...
        {
            $projhash->{'testsuites'} =
              $self->getTLDTestSuitesForProject( $projhash->{'id'},
                $get_tests );
        }
        else {
            $projhash->{'testsuites'} =
              $self->getTestSuitesForTestSuite( $projhash->{'id'}, $get_tests );
        }
...
sub _expandTree {
    ...
    foreach my $suite (@flattener) {
        ...
        for ( $j = 0 ; $j < scalar( @{ $self->{'testtree'} } ) ; $j++ ) {
            ...
            next
              unless $project
              && $self->{'testtree'}->[$j]->{'name'} eq $project;

            #Get the ball rolling if we have to
            $self->{'testtree'}->[$j]->{'testsuites'} =
              $self->getTLDTestSuitesForProject(
                $self->{'testtree'}->[$j]->{'id'}, 1 )
              if !defined( $self->{'testtree'}->[$j]->{'testsuites'} );

            #So, let's tail recurse over the testsuites.
            for (
                $i = 0 ;
                $i < scalar( @{ $self->{'testtree'}->[$j]->{'testsuites'} } ) ;
                $i++
              )
            {
                my $tailRecurseTSWalker = sub {
                    ...
                    if ( defined( $ts->{'testsuites'} )
                        && scalar( @{ $desired_ts->{'testsuites'} } ) )
                    {
                        for (
                            my $i = 0 ;
                            $i < scalar( @{ $ts->{'testsuites'} } ) ;
                            $i++
                          )
                        {
                            _tailRecurseTSWalker( $ts->{'testsuites'}->[$i],
                                $desired_ts );
                        }
                    }
                ...
                &$tailRecurseTSWalker(
                    $self->{'testtree'}->[$j]->{'testsuites'}->[$i], $suite );
        ...
        if ( !$suite->{'found'} ) {
            push( @flattener, shift @flattener )
              ; # If it wasn't found, push it on to the end of the array so the walk might find it next time.
        }
        else {
            shift @flattener;
        }
